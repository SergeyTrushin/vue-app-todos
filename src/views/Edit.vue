<template>
  <div>
    ...
    <h4>
      ...
      <button
        class="btn"
        @click="!editTitle ? (editTitle = true) : (editTitle = false)"
      >
        {{ !editTitle ? "edit" : "ok" }}
      </button>
    </h4>
    ...
    />
    <button class="add-button" @click="addTodo(taskTitle)">+</button>
    <div id="todo" v-for="todo of newNote.todos">
      <input
        type="text"
        v-if="todoConf === todo.id"
    ...
    <button
      ...
      @click="!deleteThis ? (deleteThis = true) : ''"
    >
      delete note
    </button>
    <DeleteNote
...
export default {
  ...
  data() {
    return {
      newNote: {
        ...this.$store.getters.allNotes.find(
