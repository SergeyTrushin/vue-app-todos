<template>
  <div class="background">
    <div class="modal">
      ...
      <button
        class="close-modal"
        @click="$emit('closeAddNote');clearAll();"
      >
        X
      </button>
      <hr/>
      ...
      <button class="addNote" @click="addNote">CONFIRM</button>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    toggle(id) {
      this.note.todos = this.note.todos.map((el) => {
        el.id === id ? (el.completed = !el.completed) : "";
        return el;
      });
    },
    clearAll() {
